package com.moses.cloud.orm.crypt.interceptor;

import com.moses.cloud.commons.utils.AES256Utils;
...
@Component
@ConditionalOnProperty(prefix="moses.aes256",name="enabled",havingValue="true")
public class CryptInterceptor implements Interceptor {

    private Logger logger = LoggerFactory.getLogger(CryptInterceptor.class);
...
//        Boolean isNull = isSingle == null;
//        Boolean isTrue = isSingle && !CollectionUtils.isEmpty(set);
//        Boolean isFalse = !isSingle && !CollectionUtils.isEmpty(set) && set.contains(name);
//        if (isNull || isTrue || isFalse){
//            return encryptDataToString(str);
//        }
        ...
        if (isSingle && !CollectionUtils.isEmpty(set)) {
            //todo 加密实现
//            str = "";
            return encryptDataToString(str);
        }
        if (!isSingle && !CollectionUtils.isEmpty(set) && set.contains(name)) {
            //todo 加密实现
//            str = "";
            return encryptDataToString(str);
        }
    ...
    private String stringDecrypt(String str) {
        ...
        return decryptDataToString(str);
    }
    /**
     * list 加密
     *
     ...
     * @return
     */
    public String decryptDataToString(String str) {
        return AES256Utils.decoder(str);
    }
     ...
     * @return
     */
    public String encryptDataToString(String str) {
        return AES256Utils.encode(str);
    }
}
